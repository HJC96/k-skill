...
- 카카오맵 모바일 검색(무인증): https://m.map.kakao.com/actions/searchView — `religious-facility-search`와 `public-restroom-nearby`가 결과 HTML의 `data-cid`에서 장소 id를 추출한다. 브라우저 `User-Agent`가 없으면 403.
